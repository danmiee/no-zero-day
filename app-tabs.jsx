...
function AddTask({ task, onAdd, onBack }) {
  ...
  const [title, setTitle] = useS(task ? task.t : '');
...
// ── 마이 (profile) ─────────────────────────────────────────────
function Profile({ state, onReset, onTheme, onEdit, onRemove, tab, onTab }) {
    ...
    <ScreenShell pb={0}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <Pad style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {state.tasks.length === 0 && <Card style={{ textAlign: 'center', color: 'var(--muted)', fontSize: 14 }}>아직 관리할 할 일이 없어요.</Card>}
...
function Prototype() {
  ...
  if (nav.screen === 'tab') {
    ...
    if (nav.tab === 'me') return <Profile state={state} onReset={store.reset} onTheme={store.setTheme}
  ...
  if (nav.screen === 'edit')
    return <AddTask task={nav.task} onBack={() => goTab(nav.tab || 'home')} onAdd={(data) => { store.updateTask(nav.task.id, data); goTab(nav.tab || 'home'); }} />;
